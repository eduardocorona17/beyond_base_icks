...
def main():
    ...
    while True: # Run a single turn on each iteration of the loop.
        ...
        # heck if the user has solved the puzzle:
        if SOLVED_TOWER in (towers["B"], towers["C"]):
            display_towers(towers)  # Display towers one last time.
            print("You have solved the puzzle!")
            sys.exit()

def get_player_move(towers):
    """Asks player for a move. Returns (from_tower, to_tower)."""
    ...
    while True: # Keep asking player until they enter a valid move.
        ...
        # Make sure the user entered valid tower letter:
        ...
        if len(towers[from_tower]) == 0:
            ...
            # This is a valid move, so return selected towers:
            return from_tower, to_tower
...
def display_towers(towers):
    ...
    # Display the lower labels A, B, and C:
    empty_space = " " * (TOTAL_DISKS)
    print("{0} A{0}{0} B{0}{0} C\n".format(empty_space))

def display_disk(width):
    """Display a disk of the given wdth. A width of 0 means no disk."""
    empty_space = " " * (TOTAL_DISKS - width)

    if width == 0:
        # Display a ppole segment without a disk:
